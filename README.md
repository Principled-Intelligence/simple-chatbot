...
| `--reasoning-effort`          | `None`                                       | Reasoning effort, passed to LiteLLM as-is (values are provider-specific, e.g. `none`/`minimal`/`low`/`medium`/`high`, `xhigh`, `max`, `disable`); falls back to `SIMPLE_CHATBOT_REASONING_EFFORT` |
